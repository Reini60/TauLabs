 ...
 * @{
 * @brief Defines board specific static initializers for hardware for the Naze32 board.
 *****************************************************************************/
/* 
...
static const struct flashfs_logfs_cfg flashfs_internal_cfg = {
	.fs_magic      = 0x9ae4be11,
	.total_fs_size = EE_BANK_SIZE, /* 12K bytes (12x1KB sectors) */
	.arena_size    = 0x00001000,   /* EE_BANK_SIZE / 4; 16 * slot size = 4K bytes = 4 sectors */
	.slot_size     = 0x00000100,   /* 256 bytes */

	.start_offset  = EE_BANK_BASE, /* start @111Kb */
...
static const struct pios_i2c_adapter_cfg pios_i2c_adapter_cfg = {
	.regs = I2C2,
	.init = {
...
uint32_t pios_i2c_adapter_id;
void PIOS_I2C_adapter_ev_irq_handler(void)
{
	/* Call into the generic code to handle the IRQ for this specific device */
	PIOS_I2C_EV_IRQ_Handler(pios_i2c_adapter_id);
}

void PIOS_I2C_adapter_er_irq_handler(void)
{
	/* Call into the generic code to handle the IRQ for this specific device */
	PIOS_I2C_ER_IRQ_Handler(pios_i2c_adapter_id);
}
