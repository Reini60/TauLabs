...
#define STM32F10X_FLASH_SECTOR_SIZE 1024

static bool PIOS_Flash_Internal_GetSectorInfo(uint32_t address, uint32_t *sector_start, uint32_t *sector_size)
{
	uint32_t CPUFlashSize = PIOS_SYS_getCPUFlashSize();

	// Workaround for NAZE32 with STM32F103C8T6 chip
	// showing 64kb of flash, but actually it has 128kb
	if (CPUFlashSize < 0x00010001)
		CPUFlashSize *= 2;

	if (address < FLASH_BASE || address >= FLASH_BASE + CPUFlashSize)
		return false;

	*sector_start = address & (~(STM32F10X_FLASH_SECTOR_SIZE - 1));
	*sector_size = STM32F10X_FLASH_SECTOR_SIZE;

	return true;
}

enum pios_internal_flash_dev_magic {
	PIOS_INTERNAL_FLASH_DEV_MAGIC = 0x1e6bc239,
};
